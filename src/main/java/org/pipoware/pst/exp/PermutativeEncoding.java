 ...
 */
public class PermutativeEncoding {
  
  private PermutativeEncoding() {
  }

  private static final int mpbbCrypt[]
    = {
  ...
  public static void encode(byte[] data) {
    encode_decode(data, true);
  }

  public static void decode(byte[] data) {
    encode_decode(data, false);
  }

  public static void encode_decode(byte[] data, boolean encode) {
    int offset = encode ? 0 : 512;
    for (int i = 0; i < data.length; i++) {
      data[i] = (byte) mpbbCrypt[Byte.toUnsignedInt(data[i]) + offset];
    }
  }
}
